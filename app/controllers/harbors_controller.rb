class HarborsController < ApplicationController
  before_action :set_harbor, only: [:show, :edit, :update, :destroy, :delete]
  ...
  def update
    respond_to do |format|
      if @harbor.update(harbor_params)
        format.html { redirect_to harbors_path, notice: 'Harbor was successfully updated.' }
  ...
  def destroy
    respond_to do |format|
      if params[:name] == @harbor.name
        @harbor.destroy
        format.html { redirect_to harbors_url, notice: 'Harbor was successfully destroyed.' }
        format.json { head :no_content }
      else
        format.html { redirect_to harbor_path(@harbor) + "/delete", alert: 'Incorrect name.' }
        format.json { head :no_content }
      end
    end
  end

  # GET /harbors/1/delete
  def delete
  end
